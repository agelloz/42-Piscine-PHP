...
&& ctype_alnum($_POST["login"]) && ctype_alnum($_POST["passwd"]))
{
    ...
    if ($run && !mysqli_num_rows($run))
    {
        ...
        echo "<head><link rel='stylesheet' type='text/css' href='style.css'></head><html><body><h2>Welcome ". $_POST["login"] . "! Happy shopping :)</h2></body></html>";
    }
    elseif ($run)
    {
        ...
        <html><body><h2>
            Account already existing, please try again.<br>
        </h2></body></html>";
    }
}
else
